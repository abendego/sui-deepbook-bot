...
var marginTPSL_exports = {};
__export(marginTPSL_exports, {
  MarginTPSLContract: () => MarginTPSLContract
});
module.exports = __toCommonJS(marginTPSL_exports);
  ...
  constructor(config) {
    ...
    this.newCondition = (poolKey, triggerBelowPrice, triggerPrice) => (tx) => {
      ...
      const inputPrice = Math.round(
        triggerPrice * import_config.FLOAT_SCALAR * quoteCoin.scalar / baseCoin.scalar
      );
      return tx.moveCall({
    ...
    this.newPendingLimitOrder = (poolKey, params) => (tx) => {
      const {
        ...
        isBid,
        payWithDeep = true,
        expireTimestamp = import_config.MAX_TIMESTAMP
      } = params;
      const pool = __privateGet(this, _config).getPool(poolKey);
      const baseCoin = __privateGet(this, _config).getCoin(pool.baseCoin);
      const quoteCoin = __privateGet(this, _config).getCoin(pool.quoteCoin);
